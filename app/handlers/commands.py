...
async def init(message: types.Message):
    ...
    if app_context.tesseract_reader.super_admin:
        await message.answer(text=f"Already initialized! Superadmin set: @{app_context.tesseract_reader.super_admin}")
        ...
        await message.answer(text=f"Initialized! Superadmin set: @{app_context.tesseract_reader.super_admin}")
